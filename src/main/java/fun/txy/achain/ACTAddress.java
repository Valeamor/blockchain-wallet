...
public class ACTAddress {
  ...
  public enum Type {
    ...
    public boolean lenNotEqual(int len) {
  ...
  public ACTAddress(byte[] encoded, Type type) {
    if (type.lenNotEqual(encoded.length)) {
      throw new RuntimeException("地址格式错误");
    }
  ...
  public static boolean check(String address, Type type) {
    if (address == null || !address.startsWith("ACT")) {
      return false;
    }
    try {
  ...
  public static String getMainAddress(String address) {
    if (address.length() >= 60) {
      return address.substring(0, address.length() - 32);
    } else {
      return address;
    }
  }
  ...
  public String getAddressStrStartWithSymbol() {
    return ActTransaction.ACT_SYMBOL + getAddressStr();
  }

  public String getContractStrStartWithSymbol() {
    return ActTransaction.CONTRACT_SYMBOL + getAddressStr();
  }
